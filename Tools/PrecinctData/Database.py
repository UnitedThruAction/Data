...
class Database(object):
    ...
    def __init__(self):
        ...
        # Load or create views.  See
        # https://markhaa.se/couchdb-views-in-python.html
        ...
        vtd_by_census_logrecno = ViewDefinition(
            "db_views",
            "vtd_by_census_logrecno",
            "function(doc) "
            "{if(doc.doctype == 'VTD') "
        ...
        vtd_by_census_county = ViewDefinition(
            "db_views", "vtd_by_census_county", "function(doc) "
            "{if(doc.doctype =='VTD') "
            "{emit(doc.census_COUNTY, doc._id)}}")
        vtd_by_census_county.sync(self.db)

        vtd_by_census_county_cousub_vtd = ViewDefinition(
            "db_views", "vtd_by_census_county_cousub_vtd", "function(doc) "
            "{if(doc.doctype =='VTD') "
            "{emit([doc.census_COUNTY, "
            "doc.census_COUSUB, doc.census_VTD], doc._id)}}")
        vtd_by_census_county_cousub_vtd.sync(self.db)

        er_by_county_precinct = ViewDefinition(
            "db_views", "er_by_county_precinct", "function(doc) "
            "{if(doc.doctype == 'ElectionResult') "
            "{emit([doc.oe_county_name, "
            "doc.oe_precinct], doc._id)}}")
        er_by_county_precinct.sync(self.db)

        er_doc_by_county_precinct = ViewDefinition(
            "db_views", "er_doc_by_county_precinct", "function(doc) "
            "{if(doc.doctype == 'ElectionResult') "
            "{emit([doc.oe_county_name, "
            "doc.oe_precinct], doc)}}")
        er_doc_by_county_precinct.sync(self.db)

        er_by_date_office_district = ViewDefinition(
            "db_views", "er_by_date_office_district", "function(doc) "
            "{if(doc.doctype == 'ElectionResult') "
            "{emit([doc.oe_election_date, doc.oe_office, doc.oe_district], doc._id)}}")

        er_by_date_office_district.sync(self.db)

        er_by_date_state_election_county_precinct_office_district_party_candidate = ViewDefinition(
            "db_views", "er_by_date_state_election_county_precinct_office_district_party_candidate", "function(doc) "
            "{if(doc.doctype == 'ElectionResult') "
        ...
        er_by_date_state_election_county_precinct_office_district_party_candidate.sync(
            self.db)

        ed_by_countyname = ViewDefinition(
            "db_views", "ed_by_countyname", "function(doc) "
            "{if(doc.doctype == 'ElectionDistrict') "
            "{emit(doc.vf_countyname, doc._id)}}")
        ed_by_countyname.sync(self.db)

        ed_by_county_edcode = ViewDefinition(
            "db_views", "ed_by_county_edcode", "function(doc) "
            "{if(doc.doctype == 'ElectionDistrict') "
        ...
        cousub_by_census_logrecno = ViewDefinition(
            "db_views", "cousub_by_census_logrecno", "function(doc) "
            "{if(doc.doctype == 'Cousub') "
            "{emit(doc.census_LOGRECNO, doc._id)}}")
        cousub_by_census_logrecno.sync(self.db)

        cousub_by_county_cousub = ViewDefinition(
            "db_views", "cousub_by_county_cousub", "function(doc) "
            "{if(doc.doctype == 'Cousub') "
...
class QueryType(Enum):
    ...
    ER_DOC_BY_COUNTY_PRECINCT = "db_views/er_doc_by_county_precinct"
