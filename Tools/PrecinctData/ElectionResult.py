...
class ElectionResult(Document):
    ...
    def load_single_file(filename, overwrite=False):
        ...
        with open(filename) as csvfile:
            ...
            for row in reader:
                ...
                oe_public_counter_votes = ElectionResult.safeint(row, 'public_counter_votes')
                oe_emergency_votes = ElectionResult.safeint(row, 'emergency_votes')
                oe_absentee_military_votes = ElectionResult.safeint(row, 'absentee_military_votes')
                oe_federal_votes = ElectionResult.safeint(row, 'federal_votes')
                oe_affidavit_votes = ElectionResult.safeint(row, 'affidavit_votes')
                oe_manually_counted_emergency = ElectionResult.safeint(row, 'manually_counted_emergency')
                oe_special_presidential = ElectionResult.safeint(row, 'special_presidential')
    ...
    def load_ers_from_db(query_type, key):
        ...
        uuids = [doc.value for doc in ElectionResult.DATABASE.view(query_type.value)[key]]
        if len(uuids) == 0:
            raise ValueError("No docs returned: {}, key {}"
                             .format(query_type, key))
        return [ElectionResult.load(ElectionResult.DATABASE, uuid) for uuid in uuids]
